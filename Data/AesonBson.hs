...
import GHC.Generics
import Data.AesonBson.Extracted


-- | Converts a JSON value to BSON.
bsonifyValue :: AESON.Value -> BSON.Value
bsonifyValue o@(Object obj) = decodeBsonObjId o -- Doc $ bsonify obj
...
aesonifyValue (ObjId oid) = toJSON $ toCodedObj oid -- Relies on bson to show the OID as 24 digit hex.
...
-- |Deals with the extracted Types
decodeBsonObjId :: AESON.Value -> BSON.Value
decodeBsonObjId (o@(Object k) ) = case runDecoderContentObj o of
  Error _ -> Doc $ bsonify k
  Success ob -> BSON.ObjId (fromCodedObj ob)

decodeBsonObjId v = bsonifyValue v
